...
Ошибка может возникнуть, если создать файл с бекапом настроек, а потом попробовать восстановить настройки из этого бекапа в более старой версии приложения.
Протоколы настроенные в новых версиях Amnezia, могут быть не совместимы со старой версией и наоборот.
...
Ошибка диспетчера пакетов сервера. Появляется при установке контейнера с сервером.
Обозначает, что менеджер пакетов занят, возникает после 30 попыток установить пакеты. Причиной возникновения может быть обновление пакетов.
...
либо некорректно настроить пользователя Sudo.
...
[таблице поддерживаемых протоколов для версии 4 и более новых версий]: https://amnezia.org/ru/learn-more/30_table-of-supported-devices/ 
[для версий 2.1.2-3.1.0]: https://ru-docs.amnezia.org/guides/protocols-table-v2/
