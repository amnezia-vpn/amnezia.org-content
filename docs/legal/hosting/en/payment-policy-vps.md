# Terms of Use

**Effective Date:** March 6, 2026

## 1. General Provisions
These Terms of Use (hereinafter referred to as the **"Terms"**) govern the use of hosting services provided through the Amnezia Hosting website and its related infrastructure (hereinafter referred to as the **"Service"**).

By registering an account, paying for services, or using the Service, the user (hereinafter referred to as the **"User"**) confirms their agreement with these Terms.

## 2. Description of Services
The Service provides infrastructure hosting services, including but not limited to:

- Virtual Private Servers (VPS);
- Computing resources;
- Management tools and technical support.

The Service may be modified, updated, or discontinued from time to time without prior notice.

## 3. Account and Access
To use the Service, the User may be required to create an account and provide accurate information during registration.

The User is responsible for:

- maintaining the confidentiality of account login credentials;
- all activities carried out through their account;
- using the Service in compliance with applicable laws and these Terms.

## 4. Acceptable Use
The User agrees not to use the Service for:

- illegal activities;
- distribution of malicious software;
- sending spam, phishing, or fraudulent messages;
- hosting or distributing prohibited content;
- conducting DDoS attacks or attempting to disrupt network infrastructure;
- any activities that interfere with the normal operation of the Service.

If such activities are detected, access to the Service may be suspended or terminated without prior notice.

## 5. Payment for Services
Services are provided on a prepaid basis, unless otherwise specified.

Payments may be processed through third-party payment providers. By making a payment, the User agrees to the terms and conditions of the respective payment provider.

Refund and compensation conditions are governed by a separate [Refund and Downtime Compensation Policy](/refund-policy), published on the website.

## 6. Service Availability
The Service is provided on an **"as is"** and **"as available"** basis.

Despite efforts to ensure stable operation, uninterrupted and error-free service cannot be guaranteed. Temporary interruptions may occur due to maintenance, updates, or infrastructure failures.

## 7. User Content
The User is fully responsible for any data, software, files, or materials hosted or transmitted through the Service.

The Service does not continuously monitor user content but may take action if these Terms or applicable laws are violated.

## 8. Suspension and Termination of Access
Access to the Service may be temporarily restricted or completely terminated in cases of:

- violation of these Terms by the User;
- use of the Service for illegal purposes;
- threats to the stability or security of the infrastructure.

Such measures may include account suspension, removal of hosted materials, or termination of services.

## 9. Limitation of Liability
To the maximum extent permitted by law, the Service shall not be liable for:

- indirect damages;
- loss of data;
- loss of profits;
- consequences caused by temporary unavailability of the Service.

Nothing in these Terms excludes liability that cannot be limited under applicable law.

## 10. Changes to the Terms
These Terms may be updated periodically. The current version will be published on this page with the effective date.

Continued use of the Service after changes are published constitutes acceptance of the updated Terms.

## 11. Contact Information
For questions related to these Terms, the User may contact:

**Email:** support@amnezia.host
